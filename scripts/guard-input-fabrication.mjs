...
const ROOTS = ['packages/client/src', 'packages/core/src', 'packages/mobile'];
const EXTS = new Set(['.ts', '.js', '.mjs', '.cjs', '.tsx', '.jsx', '.java']);
// `android`/`ios` = generated by `cap add`; `www` = staged renderer copy. Scan the source template,
// not the generated/copied trees.
const SKIP_DIR = new Set(['node_modules', 'dist', 'out', 'build', '.git', 'android', 'ios', 'www']);
...
const RULES = [
  ...
  // Android WebView remote debugging — same risk as the desktop CDP port (chrome://inspect injects
  // isTrusted input). Must be gated to FLAG_DEBUGGABLE so the signed release APK keeps it off; a
  // literal `true` is a release backdoor.
  { re: /setWebContentsDebuggingEnabled\s*\(\s*true\s*\)/, msg: 'hard-enables Android WebView debugging in all builds (anti-bot bypass) — gate to FLAG_DEBUGGABLE instead' },
];
